                ...
                >
                    ...
                    >
                        ...
                        <div className="absolute top-3 left-3 z-20">
                            <span className="px-3 py-1.5 text-xs font-bold bg-gradient-to-r from-neon-cyan to-neon-purple text-white rounded-full">
                                Full Stack
                            </span>
                        </div>
                        ...
                        <div className="absolute bottom-3 left-0 right-0 z-20 flex flex-col items-center gap-2">
                            <span className="px-2 py-1 bg-dark-bg/80 text-white text-xs rounded-full">
                            ...
                            <div className="flex gap-1.5">
                                {projectImages.map((_, idx) => (
                                    <button
                                        ...
                                        className={`h-1.5 rounded-full transition-all ${idx === currentIndex ? 'w-4 bg-neon-cyan' : 'w-1.5 bg-white/40'
                                            }`}
                                    />
                                ))}
                            </div>
                        ...
                        {!isHovered && (
                            <motion.div
                                ...
                                className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-neon-cyan to-neon-purple origin-left"
                            />
                        )}
                    </div>
                    ...
                    <div className="p-4 sm:p-6">
                        {/* Title */}
                        <motion.h3 variants={itemVariants} className="text-xl sm:text-2xl font-bold text-white mb-2">
                            {projectData.title}
                        </motion.h3>

                        {/* Description */}
                        <motion.p variants={itemVariants} className="text-text-muted text-sm mb-4 leading-relaxed">
                        ...
                        <motion.div variants={itemVariants} className="flex flex-wrap gap-2 mb-4">
                            {projectData.features[language].map((feature, idx) => (
                                <span key={idx} className="text-xs text-white/80 bg-white/5 px-2 py-1 rounded-full">
                                    {feature}
                                </span>
                            ))}
                        </motion.div>
                            ...
                            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                                {projectData.techStack.map((stack, idx) => (
                                    <div key={idx} className="flex-shrink-0">
                                        <span className={`text-xs font-medium ${colorMap[stack.color].text} block mb-1.5`}>
                                            {stack.category}
                                        </span>
                                        <div className="flex gap-1">
                                            {stack.items.map((tech, i) => (
                                                <span
                                                    key={i}
                                                    className={`px-2 py-0.5 text-xs ${colorMap[stack.color].bg} ${colorMap[stack.color].text} border ${colorMap[stack.color].border} rounded whitespace-nowrap`}
                                                >
                                                    {tech}
                                                </span>
                                            ))}
                        ...
                        <motion.div variants={itemVariants} className="flex gap-3">
                            <a
                                ...
                                className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-neon-cyan to-neon-purple text-white text-sm font-semibold rounded-lg hover:shadow-lg hover:shadow-neon-cyan/20 transition-all"
                            >
                            ...
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 border border-white/20 text-white/80 hover:text-white text-sm font-semibold rounded-lg hover:border-white/40 transition-all"
                            >
