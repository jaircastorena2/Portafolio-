    ...
    return (
        <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.5 }} className="fixed inset-0 z-[100] bg-dark-bg flex items-center justify-center overflow-hidden">
            <div className="absolute inset-0 pointer-events-none">
                <div className="absolute top-1/4 left-1/4 w-48 md:w-72 h-48 md:h-72 bg-neon-cyan/10 rounded-full blur-3xl" />
                <div className="absolute bottom-1/4 right-1/4 w-48 md:w-72 h-48 md:h-72 bg-neon-purple/10 rounded-full blur-3xl" />
            </div>
            ...
            <div className="text-center relative z-10 px-6">
                <motion.h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-white mb-4 tracking-wide">
                    {welcomeText.split('').map((letter, index) => (
                        <motion.span key={index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.2 + index * 0.05 }} className="inline-block" style={{ textShadow: '0 0 30px rgba(0, 245, 255, 0.5)' }}>{letter}</motion.span>
                    ))}
                </motion.h1>
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="text-text-muted text-sm md:text-base mb-8 tracking-widest uppercase">
